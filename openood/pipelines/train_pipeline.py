...
from openood.datasets import get_dataloader, get_feature_dataloader
...
class TrainPipeline:
    ...
    def run(self):
        ...
        try:
            ...
            loader_dict = get_feature_dataloader(self.config)
        ...
        for epoch_idx in range(1, self.config.optimizer.num_epochs + 1):
            ...
            if self.config.trainer.name == 'mos':
                ...
                if self.config.scheduler.name == 'timm' and self.config.scheduler.sched == "cosine":
                    trainer.scheduler.step(epoch_idx, val_metrics['acc'])
                    print(val_metrics, "stepping scheduler")
            comm.synchronize()
            if comm.is_main_process():
