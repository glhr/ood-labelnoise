...
class ResNet18_32x32(nn.Module):
    ...
    def get_layer_from_idx(self, layer_index):
        if layer_index == 1:
            return self.layer1
        elif layer_index == 2:
            return self.layer2
        elif layer_index == 3 or layer_index == -2:
            return self.layer3
    ...
    def intermediate_forward(self, x, layer_index):
        ...
        out = self.layer3(out)
        if layer_index == 3 or layer_index == -2:
            return out

        out = self.layer4(out)
        if layer_index == 4 or layer_index == -1:
            return out
    ...
    def intermediate_feature_to_logits(self, x, next_layers_idx=[]):
        for layer_idx in next_layers_idx:
            x = self.get_layer_from_idx(layer_idx)(x)

        feature5 = self.avgpool(x)
        feature = feature5.view(feature5.size(0), -1)
        logits_cls = self.fc(feature)
