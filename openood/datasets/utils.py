...
def get_feat_dataset(feat_path: str):
    # load in the cached feature
    loaded_data = load(feat_path, allow_pickle=True).item()
    feat = torch.from_numpy(loaded_data['feat']).to(torch.float32)
    labels = torch.from_numpy(loaded_data['label'])
    del loaded_data
    # reshape the vector to fit in to the network
    #feat.unsqueeze_(-1).unsqueeze_(-1)
    #labels.unsqueeze_(-1).unsqueeze_(-1)
    # let's see what we got here should be something like:
    # torch.Size([total_num, channel_size, 1, 1])
    print('Loaded feature size: {}'.format(feat.shape))

    dataset = FeatDataset(feat=feat, labels=labels)
    return dataset
...
def get_feature_dataloader(dataset_config: Config):
    ...
    for split in dataset_config.split_names:
        ...
        dataset = get_feat_dataset(split_config.feat_path)
