...
from .datasets import DATA_INFO, get_id_ood_dataloader, get_feat_dataloader_test
...
class Evaluator:
    def __init__(
        ...
        if id_name not in DATA_INFO:
            ...
            print(f'Dataset [{id_name}] is not supported...trying anyway')
        ...
        try:
            ...
            print(e)
            print('Trying to load feature data instead...')

            dataloader_dict = get_feat_dataloader_test(id_name, data_root,
                                                     **loader_kwargs)

        # wrap base model to work with certain postprocessors
        if 'react' in postprocessor_name:
            net = ReactNet(net)
        elif 'ash' in postprocessor_name:
            net = ASHNet(net)

        
        self.APS_results = None
        self.id_name = id_name
        self.net = net
        self.preprocessor = preprocessor
        self.postprocessor = postprocessor
        self.dataloader_dict = dataloader_dict
        self.id_loader_split = id_loader_split
        self.metrics = {
            'id_acc': None,
            'id_acc_train': None,
            'id_acc_val': None,
    ...
    def prepare_everything(self):
        ...
        # postprocessor setup
        self.postprocessor.setup(self.net, self.dataloader_dict['id'], self.dataloader_dict['ood'], id_loader_split=self.id_loader_split)


        if (self.postprocessor.APS_mode
                and not self.postprocessor.hyperparam_search_done):
            self.APS_results = self.hyperparam_search()
    ...
    def eval_acc(self, data_name: str = 'id') -> float:
        if data_name == 'id':
            if self.metrics['id_acc'] is not None:
                ...
                self.metrics['id_acc'] = test_acc
                #return test_acc

            for split in ['train', 'val']:
                if self.metrics[f'id_acc_{split}'] is not None:
                    return self.metrics[f'id_acc_{split}']
                else:
                    all_preds, all_labels = self._classifier_inference(
                        self.dataloader_dict['id'][split], f'ID Acc Eval {split}')
                    assert len(all_preds) == len(all_labels)
                    correct = (all_preds == all_labels).sum().item()
                    self.metrics[f'id_acc_{split}'] = correct / len(all_labels) * 100
            return test_acc
        elif data_name == 'csid':
    ...
    def _eval_ood(self,
        ...
        for dataset_name, ood_dl in self.dataloader_dict['ood'][
            ...
            try:
                ...
                print(f"WARNING: {dataset_name} dataset failed to compute metrics - check that dataset exists")
    ...
    def hyperparam_search(self):
        ...
        return self.postprocessor.get_hyperparam()
