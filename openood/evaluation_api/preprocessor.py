...
from openood.preprocessors import BasePreprocessor, NoPreprocessor
...
default_preprocessing_dict = {
    'clothing1M': {
        'pre_size': 256,
        'img_size': 224,
        'normalization': [[0.7249, 0.6878, 0.6702], [0.2514, 0.2656, 0.2657]],
    },
    'cifar10': {
        'pre_size': 32,
        'img_size': 32,
...
def get_default_preprocessor(data_name: str):
    ...
    if data_name.split("_")[0] not in default_preprocessing_dict:
        #raise NotImplementedError(f'The dataset {data_name} is not supported')
        return NoPreprocessor()

    config = Config(**default_preprocessing_dict[data_name.split("_")[0]])
